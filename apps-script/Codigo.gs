 ...
 *   "Atacado"   -> DRE Consolidado | Real <período> | AV | Orçado <período> | AV | AH | Real <período-1> | AV | AH
 *                  (mesmo formato da base Lynkz Gerencial Consolidado; atualize os
 *                  valores dessas colunas a cada fechamento de período)
...
// Lê a aba Atacado (Lynkz Gerencial Consolidado) crua: cabeçalho + linhas.
// O front-end identifica as colunas "Real <ano>" pelo texto do cabeçalho.
function _lerAtacado(ss, sheetName) {
  ...
  return {
    header: data[0] || [],
    rows: data.slice(1).filter(function (r) { return r[0]; }),
  };
}
